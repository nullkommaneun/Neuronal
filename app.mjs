// app.mjs (Finaler, robuster Code mit Loop-Fehlerbehandlung)
...
async function gameLoop() {
    try { // NEU: Fehlerbehandlung für den Laufzeitbetrieb
        ...
        if (trainingPhase === 1) {
            ...
            if (collisionLossHistory.length > STABILITY_PERIOD) {
                ...
                if (avgLoss < 0.01) {
                    ...
                    log("WECHSLE ZU PHASE 2: Fläche maximieren!");
                }
            }
        }
        ...
        // Rufe den nächsten Frame nur bei Erfolg auf
        requestAnimationFrame(gameLoop);

    } catch (error) {
        // Fängt Laufzeitfehler ab und stoppt die Animation sicher
        log(`FATALER FEHLER in der Game-Loop: ${error.message}`, true);
        console.error("Fehler in gameLoop:", error); // Für den Fall, dass doch eine Konsole da ist
    }
}
...
async function main() {
    // Dieser Teil ist bereits erfolgreich getestet und bleibt unverändert
    try {
        ...
        log("<br><strong style='color: #28a745;'>Initialisierung komplett. Starte Anwendung...</strong>");
        gameLoop();
    } catch (error) {
        ...
        else if (error.message.includes("Sofa")) updateDiagStatus('diag-ai', 'error');
    }
}

main();
// app.mjs (Finaler, vollständiger Code)
import { Corridor } from './corridor.mjs';
import { Sofa } from './sofa.mjs';
