document.addEventListener('DOMContentLoaded', () => {
    ...
    const runInitialDiagnostics = () => {
        setBadgeStatus('badge-js', 'success', 'JS OK'); log('JavaScript engine started.');
        if (typeof Corridor === 'undefined' || typeof Corridor.calculateCollision !== 'function') { setBadgeStatus('badge-corridor', 'error', 'Korridor-Modul FEHLT'); return false; }
        setBadgeStatus('badge-corridor', 'success', 'Korridor OK'); log('Corridor module loaded successfully.');
        if(typeof tf==='undefined'){setBadgeStatus('badge-tfjs','error','TF.js Failed');return false;}
        setBadgeStatus('badge-tfjs','warn','TF.js Geladen');log(`TensorFlow.js version: ${tf.version.tfjs}`);
    ...
    const main = async () => {
        ...
        if (!runInitialDiagnostics()) { alert("Kritischer Fehler beim Initialisieren der Module. Siehe Log für Details."); return; }
        setTimeout(async () => {
            ...
            draw(); // Zeichne den KORREKTEN Korridor beim Start
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const runInitialDiagnostics = () => {
        ...
        // NEUER CHECK: Wurde das Corridor-Modul geladen?
        if (typeof Corridor === 'undefined' || typeof Corridor.calculateCollision !== 'function') {
            setBadgeStatus('badge-corridor', 'error', 'Korridor-Modul FEHLT');
            log('Corridor module not found. Check if corridor.js is loaded correctly.', 'error');
            return false;
        }
        setBadgeStatus('badge-corridor', 'success', 'Korridor OK');
        log('Corridor module loaded successfully.');
    ...
    // Die alte 'calculateCollision' Funktion wird hier GELÖSCHT.
    ...
    const trainStep = () => tf.tidy(() => {
        const { grads, value } = optimizer.computeGradients(() => {
            ...
            for (let i = 0; i <= CONFIG.COLLISION_STEPS; i++) {
                const t = i / CONFIG.COLLISION_STEPS; const angle=-Math.PI/2*t; const offsetX=t<0.5?0:(t-0.5)*2; const offsetY=t>0.5?0:(0.5-t)*2;
                const transformedCoords = transformPoints(gridCoords, angle, offsetX, offsetY);
                // NEU: Aufruf der ausgelagerten Funktion
                totalCollision = tf.add(totalCollision, Corridor.calculateCollision(transformedCoords, shapeOutput));
            }
            const collisionLoss = tf.mul(totalCollision, CONFIG.LAMBDA_COLLISION);
            return tf.sub(tf.mul(area, -1), collisionLoss);
    ...
    const draw = () => {
        ...
        // NEU: Verwendung der Korridorbreite aus dem Modul
        const scale=CONFIG.CANVAS_SIZE/2.5; const center=CONFIG.CANVAS_SIZE/2; const w=Corridor.width*scale;
