...
async function loop(){
  try{
    for(let i=0;i<30;i++){ await stepOnce(); document.getElementById('iter').textContent = String(Number(document.getElementById('iter').textContent)||0 + 1); }
    requestAnimationFrame(loop);
  }catch(e){
    EM.log.push({type:'loop', msg:String(e), stack:e.stack||null});
    EM.fatal=true; // Hard stop
  }
}
